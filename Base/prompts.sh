...
REPOS=$(whiptail --nocancel --separate-output --checklist "Select which Repos you want to use" 0 0 5 $REPOLIST 3>&1 1>&2 2>&3 | sed 's|"||g')
...
DESKTOP=$(whiptail --nocancel --noitem --menu "Which Desktop Do You Want?" 0 0 0 $(find ./{$REPOS}/Desktops/* -maxdepth 0 -type d  -printf '%f ​ ') 3>&1 1>&2 2>&3)
CONFIGS=$( [[ ! $( find ./{$REPOS}/Desktops/$DESKTOP/Configs) ]] && echo "None" || echo $(whiptail --nocancel --noitem --menu "Do You Want Customized $DESKTOP Configs?" 0 0 0 None ​ $(find ./*/Desktops/$DESKTOP/Configs/* -maxdepth 0 -type d -printf '%f ​ ') 3>&1 1>&2 2>&3))
if [[ $CONFIGS != "None" ]] then
	CHAOTIC=$(whiptail --yesno "Do you want to use Chaotic AUR?" 0 0 0 3>&1 1>&2 2>&3 && echo true || echo false)
fi
$(. ./{$REPOS}/Desktops/$DESKTOP/prompts.sh || true) || echo "./{$REPOS}/Desktops/$DESKTOP/prompts.sh NOT FOUND"
$(. ./{$REPOS}/Desktops/$DESKTOP/Configs/$CONFIGS/prompts.sh || true) || echo "./{$REPOS}/Desktops/$DESKTOP/Configs/$CONFIGS/prompts.sh NOT FOUND"
